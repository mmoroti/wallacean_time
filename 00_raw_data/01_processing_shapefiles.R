...
# load shapefile
rm(list=ls()); gc()
shape_data <- st_read(here::here("Shapefiles",
                                 "MAMMALS_TERRESTRIAL_ONLY",
                                 "MAMMALS_TERRESTRIAL_ONLY.shp"))
# check data
names(shape_data)
nrow(shape_data)
unique(shape_data$legend) # only extant (resident)
table(shape_data$legend) # only extant (resident)
...
shape_combined <- shape_data %>%
  filter(legend == "Extant (resident)") %>%
  group_by(sci_name) %>%
  summarize(geometry = st_combine(geometry))

length(unique(shape_data$sci_name)) # 5637 nomes unicos
length(unique(shape_combined$sci_name)) # 5588, diferenca de 49 nomes
...
shape_data_valid <- shape_combined %>%
...
mammals_iucn_key <- shape_data_valid %>% 
...
mammals_shape_key <- left_join(shape_data_valid, 
...
amphibia_shape_data <- st_read(here::here("Shapefiles",
                                 "AMPHIBIANS",
...
# we need a list with unique species with one geometry and only extant (resident)
# st_union doesn't work, it's necessary check invalid geometries when using st_combine
amphibia_shape_combined <- amphibia_shape_data %>%
  filter(legend == "Extant (resident)") %>%
  group_by(sci_name) %>%
  summarize(geometry = st_combine(geometry))

length(unique(amphibia_shape_data$sci_name)) # 3988 nomes unicos
length(unique(amphibia_shape_combined$sci_name)) # 3896
...
# plot(shape_data_valid %>% filter(sci_name == "Bos gaurus"))
# Corrija as geometrias inválidas
amphibia_shape_data_valid <- amphibia_shape_combined %>%
  mutate(geometry = st_make_valid(geometry))
...
# Search by the speciesKey in rgbif
amphibia_iucn_key <- amphibia_shape_data_valid %>% 
  pull("sci_name") %>% # use fewer names if you want to just test 
  name_backbone_checklist()
...
amphibia_shape_key <- left_join(amphibia_shape_data_valid, 
...
reptilia_shape_data <- st_read(here::here("Shapefiles",
                                          "REPTILES",
                                          "REPTILES_PART2.shp"))
# check data
...
# we need a list with unique species with one geometry and only extant (resident)
# st_union doesn't work, it's necessary check invalid geometries when using st_combine
reptilia_shape_combined <- reptilia_shape_data %>%
  filter(legend == "Extant (resident)") %>%
  group_by(sci_name) %>%
  summarize(geometry = st_combine(geometry))

length(unique(reptilia_shape_data$sci_name)) # 4932 nomes unicos
length(unique(reptilia_shape_combined$sci_name)) # 4905
...
# plot(shape_data_valid %>% filter(sci_name == "Bos gaurus"))
# Corrija as geometrias inválidas
reptilia_shape_data_valid <- reptilia_shape_combined %>%
  mutate(geometry = st_make_valid(geometry))
...
# Search by the speciesKey in rgbif
reptilia_iucn_key <- reptilia_shape_data_valid %>% 
  pull("sci_name") %>% # use fewer names if you want to just test 
  name_backbone_checklist()
...
reptilia_shape_key <- left_join(reptilia_shape_data_valid, 
...
birds_shape_data <- st_read(here::here("Shapefiles",
                                          "REPTILES",
                                          "REPTILES_PART2.shp"))
# check data
...
tetrapod_shapefile <- rbind(mammals_shape_key,
      amphibia_shape_key, 
      reptilia_shape_key)
      #birds_shape_key,)
