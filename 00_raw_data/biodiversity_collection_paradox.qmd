...
data_institutions <- read.delim(file.path("00_raw_data", "raw_data", "institutions.tsv"))
data_country <- readxl::read_xlsx(file.path("00_raw_data", "raw_data", "GlobalNorthVsSouth.xlsx"))
...
#)
write_parquet(tetrapods_data, file.path("00_raw_data", "tetrapods_data.parquet"))
```
...
tetrapods_data <- read_parquet(
  file.path("00_raw_data", "tetrapods_data.parquet"))
...
```{r, save filter data}
write_parquet(without_duplicates_df, file.path("00_raw_data", "tetrapods_without_duplicates.parquet"))
```

# Part 3. Obtain quantity of terrestrial tetrapods are deposited 
```{r}
without_duplicates_df <- read_parquet(
  file.path("00_raw_data", "tetrapods_without_duplicates.parquet"))
...
dados_north_south <- left_join(data_country, without_duplicates_df_num, by= "CountryName")
View(dados_north_south)
